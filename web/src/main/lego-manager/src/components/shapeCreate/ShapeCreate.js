...
class ShapeCreate extends Component {
  ...
  submit(values) {
    this.props.addShape(values)
      .then(r => (Link.redirect('/shape/' + r.value.data.id)))
  }

  render() {
    const {handleSubmit} = this.props
    return (<Paper className="ShapeCreate" zDepth={1}>
      ...
      <form className="ShapeCreate-form" onSubmit={handleSubmit(vals => this.submit(vals))}>
        <Field className="ShapeCreate-item" name="name" component={TextField} hintText="Name" validate={[required]}/>
...
let component = connect(store => ({}), dispatch => bindActionCreators({
  ...actions
}, dispatch))(ShapeCreate)

component = reduxForm({form: 'shapeCreate'})(component)
